...
public class OverdueTaskViewModel extends BaseViewModel {
    ...
    private TaskRepository mResitory;

    public SingleLiveEvent<Integer> showStatus = new SingleLiveEvent<>();

    public OverdueTaskViewModel(@NonNull Application application) {
        super(application);
        mResitory = new TaskRepository();
    }

    public void getData() {
        List<TaskBean> task = mResitory.getTaskByStatus(0, Integer.MAX_VALUE, System.currentTimeMillis(), false);
